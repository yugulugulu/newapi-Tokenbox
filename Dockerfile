...
RUN test -f /build/web/dist/index.html || (echo "Frontend build failed: index.html not found" && exit 1)
...
# Copy docs to web/dist
RUN mkdir -p /build/web/dist/docs && \
    cp -r /build/docs/.vitepress/dist/* /build/web/dist/docs/
# Verify docs were copied successfully
RUN test -f /build/web/dist/docs/index.html || (echo "Docs copy failed: docs/index.html not found" && exit 1)
